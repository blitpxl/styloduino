...
from .util import get_screen_size, translate
...
class TitleBar(QtWidgets.QFrame):
    def __init__(self, p=None):
        ...
        self.closeButton.setToolTip(translate("window_control_close", "Close"))
        self.sizeButton.setToolTip(translate("window_control_size", "Maximize/Restore"))
        self.minimizeButton.setToolTip(translate("window_control_minimize", "Minimize"))
...
class WindowContainer(QWidget):
    ...
    def windowCloseEvent(self) -> None:
        ...
        self.hideAnimation.finished.connect(self.exec_cleanup)
        self.hideAnimation.finished.connect(self.close)  # noqa (false warning)
    ...
    def exec_startup(self):
        self.isStartup = False
        for item in self.windowObject.startupQueue:
            if isinstance(item, QObject):
                item.close()
            else:
                if callable(item):
                    ...
                    raise Exception(f"Item '{item}' in startup queue is not an instance of QObject, "
                                    f"nor is it a callable.")

    def exec_cleanup(self):
        for item in self.windowObject.cleanupQueue:
            if isinstance(item, QObject):
                item.close()
            else:
                if callable(item):
                    ...
                    raise Exception(f"Item '{item}' in cleanup queue is not an instance of QObject, "
                                    f"nor is it a callable.")
    ...
    def showEvent(self, event) -> None:
        ...
        self.showAnimation.finished.connect(lambda: self.exec_startup() if self.isStartup else lambda: None)
